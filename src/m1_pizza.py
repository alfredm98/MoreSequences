...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(number_of_slices):
        center = seq[k]
        line = rg.Line(circle.center, center)
        line.color = color
...
def run_test_polygon():
    ...
    title = 'Polygon test 3: 10 segments with thin red lines'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(100,100), 50)
    circle.outline_thickness = 1
    polygon(window, circle, 10, 'red', 1)
    window.close_on_mouse_click()
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'blue'
    fancy_polygon(window, circle, 10, 2, 'red', 2)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(number_of_lines):
        if k + hops_to_next_point < number_of_lines:
            start = seq[k]
            end = seq[k + hops_to_next_point]
        else:
            start = seq[k]
            end = seq[k + hops_to_next_point - number_of_lines]
        line = rg.Line(start, end)
        line.arrow = 'last'
        line.color = color
        line.thickness = thickness
